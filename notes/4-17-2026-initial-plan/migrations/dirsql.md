...
## Decisions confirmed at migration time

- **Drop cargo-dist entirely.** Consumption happens via pip / npm /
  cargo only, not via direct GitHub archive downloads. No `.tar.xz` /
  `.zip` assets on the GH Release. The autogenerated cargo-dist
  `release.yml` is deleted in step 7 along with the rest of the old
  release tooling.
- **Keep the 02:00 UTC daily cron.** Same schedule as today's
  `patch-release.yml`. The target `release.yml` above already uses
  `"0 2 * * *"`.

## One call still left for the dirsql side

**Keep or drop the `@dirsql/cli-{target}` scoped naming?** Switching to
