...
describe('piot-ci actionlint-idtoken (integration)', () => {
  it('passes (exit 0, no output) when no PR-time file grants id-token: write', async () => {
    files({
      '.github/workflows/build.yml': 'permissions:\n  contents: read\n',
      '.github/workflows/_matrix.yml': 'jobs:\n  build:\n    runs-on: ubuntu-latest\n',
      '.github/workflows/check.yml': 'permissions:\n  contents: read\n',
    });
    await expect(actionlint()).resolves.toBe(0);
    expect(out.join('')).toBe('');
  });

  it('fails, echoing the line number and an ::error, when a file grants id-token: write', async () => {
    files({
      '.github/workflows/build.yml': 'permissions:\n  contents: read\n',
      '.github/workflows/_matrix.yml': 'permissions:\n  id-token: write\n',
      '.github/workflows/check.yml': 'permissions:\n  contents: read\n',
    });
    await expect(actionlint()).resolves.toBe(1);
    expect(out.join('')).toBe(
      '2:  id-token: write\n' +
  ...
  it('does not match an in-comment mention of id-token: write', async () => {
    files({
