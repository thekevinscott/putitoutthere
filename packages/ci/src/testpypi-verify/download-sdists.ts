 ...
 * `-{version}.tar.gz`, and downloaded it — retrying up to `MAX_ATTEMPTS`
 * times, sleeping `attempt*10`s, and failing on the last with `failed to
 * download sdist for {req}: {exc}`. The HTTP GETs run through `curl` (the same subprocess
