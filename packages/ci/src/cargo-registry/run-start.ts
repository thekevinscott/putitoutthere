...
export async function runCargoRegistryStart(): Promise<number> {
  ...
  await appendFile(githubEnv, `CARGO_HTTP_REGISTRY_PID=${child.pid}\n`);

  let ready = false;
  for (let attempt = 1; attempt <= MAX_START_ATTEMPTS; attempt++) {
    try {
      await execCapture('curl', ['-fsS', '-o', '/dev/null', ENDPOINT]);
  ...
  if (decision.errorLine !== null) {
    ...
    process.stdout.write((await readRaw(logPath)) ?? '');
    return decision.exitCode;
  }

  await mkdir(`${home}/.cargo`, { recursive: true });
  await appendFile(`${home}/.cargo/config.toml`, CONFIG_APPEND);
  return decision.exitCode;
}
