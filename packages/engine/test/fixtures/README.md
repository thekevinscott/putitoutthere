...
| `polyglot-everything/` | crates `-rust` + pypi `-python` + multi-mode npm `@putitoutthere/-cli` (bundled-cli + napi-rs `.node`, both selected via `optionalDependencies`) — multi-mode polyglot shape |
