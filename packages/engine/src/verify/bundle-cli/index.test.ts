...
beforeEach(() => {
  out.length = 0;
  entryNames = [];
  readPythonSourceMock.mockResolvedValue('');
  findDistFileMock.mockResolvedValue(WHEEL);
...
describe('verifyBundleCli', () => {
  it('passes when the wheel contains <stage_to>/<bin> (absolute path)', async () => {
    entryNames = ['demo/__init__.py', 'dirsql/_binary/dirsql'];
    const code = await verifyBundleCli(opts());
  ...
  it('resolves a relative --path against cwd', async () => {
    ...
    const code = await verifyBundleCli(opts({ cwd: '/work', path: 'pkg', stageTo: 'stage/bin', bin: 'tool' }));
  ...
  it('appends .exe on a Windows target', async () => {
    ...
    const code = await verifyBundleCli(opts({ stageTo: 'stage/bin', bin: 'tool', target: 'x86_64-pc-windows-msvc' }));
    expect(out.join('')).toContain('ok bundle_cli: stage/bin/tool.exe present in');
    expect(code).toBe(0);
  });

  it('subtracts [tool.maturin].python-source before matching', async () => {
    readPythonSourceMock.mockResolvedValue('python');
    entryNames = ['dirsql/_binary/dirsql'];
    const code = await verifyBundleCli(opts({ stageTo: 'python/dirsql/_binary' }));
    expect(out.join('')).toContain('ok bundle_cli: dirsql/_binary/dirsql present in');
    expect(code).toBe(0);
  });

  it('fails and lists contents when the binary is missing', async () => {
  ...
  it('fails when no wheel is produced under dist/', async () => {
    findDistFileMock.mockResolvedValue(null);
    const code = await verifyBundleCli(opts());
