...
// #317: pre-merge `check` subcommand. The PR-time reusable workflow
// (`.github/workflows/check.yml`, also #317) calls into the same engine
// entry point that `plan` uses for static validation, but stops short of
// emitting a build matrix. The acceptance criteria for #317 are:
//
//   - the subcommand exists and dispatches into config validation,
//   - on a valid `putitoutthere.toml` it exits 0 with a human summary,
//   - on an invalid config it exits non-zero with the same error shape
//     consumers already see from `plan` (issue's "shares the engine
//     entry point with `release.yml`" clause),
//   - it does NOT emit `matrix=...` to `$GITHUB_OUTPUT` — that's the
//     trait that distinguishes pre-merge validation from `plan`. A
//     `matrix=` line in the PR-time path would cascade into the build
//     job and surface a class of release-side work at PR time, which
//     is exactly the "parallel diagnostic surfaces" trap non-goal #8
//     forbids.
describe('#317 cli: check', () => {
  let repo: string;
  ...
  beforeEach(() => {
    repo = mkdtempSync(join(tmpdir(), 'cli-check-'));
    git(['init', '-q', '-b', 'main']);
    git(['config', 'user.email', 'test@example.com']);
    git(['config', 'user.name', 'Test']);
    git(['config', 'commit.gpgsign', 'false']);
...
`,
  ...
  it('is a registered subcommand (not "unknown command")', async () => {
    const stderrChunks: string[] = [];
    vi.spyOn(process.stderr, 'write').mockImplementation((chunk) => {
      stderrChunks.push(typeof chunk === 'string' ? chunk : chunk.toString());
      return true;
    });
    const code = await run(['node', 'putitoutthere', 'check', '--cwd', repo]);
    // Whatever the outcome, "unknown command" is the failure mode the
    // subcommand-registration test guards against; without `check`
    // wired in cli.ts, dispatch falls through to the unknown-command
    // arm regardless of cwd contents.
    expect(stderrChunks.join('')).not.toMatch(/unknown command/);
    expect(code).toBe(0);
  });

  it('exits 0 with a human summary on a valid putitoutthere.toml', async () => {
    const stdoutChunks: string[] = [];
    vi.spyOn(process.stdout, 'write').mockImplementation((chunk) => {
      stdoutChunks.push(typeof chunk === 'string' ? chunk : chunk.toString());
      return true;
    ...
    const code = await run(['node', 'putitoutthere', 'check', '--cwd', repo]);
    expect(code).toBe(0);
    // Some humanly readable acknowledgement — we don't pin the exact
    // wording, but a green check from a consumer's CI log needs *some*
    // observable "yes, this passed" line. The packages count or an "OK"
    // word satisfies the contract.
    expect(stdoutChunks.join('')).toMatch(/check|OK|passed|demo/i);
  });

  it('exits 1 with a friendly error on a malformed putitoutthere.toml', async () => {
    // Same surface as `plan` rejects today (common-mistakes pass +
    // zod parse). The whole point of #317 is to surface this at PR
    // time instead of release time; the contract is "consumer sees
    // the same diagnosable error, earlier".
    writeFileSync(
      join(repo, 'putitoutthere.toml'),
      `[putitoutthere]
version = 1
...
`,
    ...
    expect(code).toBe(1);
    expect(stderrChunks.join('')).toMatch(/^putitoutthere:/m);
    // The common-mistakes pass names both the typo and the fix; pin
    // the hint shape so a regression in detectCommonMistakes silently
    // dropping `check`'s coverage gets caught.
    expect(stderrChunks.join('')).toMatch(/\[\[package\]\]/);
  });

  it('does NOT write matrix= to $GITHUB_OUTPUT', async () => {
    // This is the distinguishing trait between `check` and `plan`. The
    // PR-time workflow calls into the engine to surface config issues,
    // not to scaffold a build matrix; emitting a `matrix=` output here
    // would let the downstream "no per-check workflow inputs" surface
    // (the consumer's `jobs.<id>.outputs.matrix` reference shape from
    // release.yml / build.yml) latch onto check.yml too and would re-
    // create a parallel diagnostic surface (non-goal #8).
    const outFile = join(repo, 'gha-output.txt');
    writeFileSync(outFile, '', 'utf8');
    process.env.GITHUB_OUTPUT = outFile;
    ...
    expect(code).toBe(0);
    const out = readFileSync(outFile, 'utf8');
    expect(out).not.toMatch(/^matrix=/m);
  });
});
