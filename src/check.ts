...
export function runChecks(opts: CheckOptions): CheckFinding[] {
  ...
  checkCratesPackageSize(packages, findings);
...
// crates.io rejects any `.crate` upload over this size with
// `413 Payload Too Large`. cargo surfaces the rejection only after
// `cargo publish` has run the verification build and compiled every
// transitive dep; `cargo package` reproduces the identical tarball at
// PR time in seconds.
...
function checkCratesPackageSize(
  packages: readonly Package[],
  findings: CheckFinding[],
): void {
  for (const p of packages) {
