...
- **PyPI auth-failure error embeds an OIDC probe summary and a stable `PIOT_AUTH_NO_TOKEN` code.** When neither OIDC trusted publishing nor `PYPI_API_TOKEN` resolves, the thrown error now includes a per-probe result block (was OIDC env present? did the id-token request succeed? did the registry mint exchange? was `PYPI_API_TOKEN` set?) and the body excerpt from any registry rejection (e.g. PyPI's 422 `invalid-publisher` with the expected `job_workflow_ref` list). The error message is tagged `[PIOT_AUTH_NO_TOKEN]` and the auth-guide URL carries `?code=PIOT_AUTH_NO_TOKEN` for code-specific deep-linking. Foreign agents reading the public run log can fingerprint on the code instead of free-form prose. No effect on successful publishes.
